...
-- -----------------------------------------------------
-- Placeholder table for view `shopdb`.`INVOICE_CUSTOMER`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `shopdb`.`INVOICE_CUSTOMER` (`INV_NUMBER` INT, `INV_DATE` INT, `INV_STATUS` INT, `INV_TOTAL` INT, `CUS_CODE` INT, `CUS_LNAME` INT, `CUS_PHONE` INT, `CUS_WALLET` INT, `EMP_CODE` INT);

-- -----------------------------------------------------
-- Placeholder table for view `shopdb`.`INVOICE_LINE`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `shopdb`.`INVOICE_LINE` (`INV_NUMBER` INT, `INV_DATE` INT, `INV_STATUS` INT, `INV_TOTAL` INT, `LINE_NUMBER` INT, `P_CODE` INT, `P_DESCRIPT` INT, `LINE_UNITS` INT, `LINE_PRICE` INT, `EMP_CODE` INT);
...
BEGIN
	IF p_keyword IS NULL OR TRIM(p_keyword) = '' THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'KEYWORD CANNOT BE BLANK.';
	END IF;
...
BEGIN
    ...
    IF (SELECT INV_STATUS FROM INVOICE WHERE INV_NUMBER = p_inv_number) <> 'DRAFT' THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'CANNOT PAY PAID OR CANCELLED INVOICE.';
	END IF;
    
    IF NOT EXISTS (SELECT 1 FROM LINE WHERE INV_NUMBER = p_inv_number) THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'CANNOT PAY INVOICE WITH NO LINE.';
	END IF;
    ...
    IF EXISTS 
        ...
        WHERE INV_NUMBER = p_inv_number AND LINE_UNITS > P_QOH) 
	THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'INSUFFICIENT QUANITY ON HAND. CHECK INVOICE DETAILS.';
	END IF;
...
BEGIN
    ...
    IF (SELECT INV_STATUS FROM INVOICE WHERE INV_NUMBER = p_inv_number) <> 'DRAFT' THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'CAN ONLY PAY DRAFT INVOICE. CHECK INVOICE STATUS.';
	END IF;
    ...
    IF p_pay_ref IS NULL OR  TRIM(p_pay_ref) = '' THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'BANK TRANSFER REFERENCE IS REQUIRED.';
	END IF;
...
BEGIN
	...
	THEN SET v_rev_isvalid = 0; ELSE SET v_rev_isvalid = 1; END IF;
    
    INSERT INTO PRODUCT_REVIEW (CUS_CODE, P_CODE, REV_RATING, REV_COMMENT, REV_ISVALID)
    VALUES (p_cus_code, p_p_code, p_rev_rating, p_rev_comment, v_rev_isvalid);
...
-- -----------------------------------------------------
-- View `shopdb`.`INVOICE_CUSTOMER`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `shopdb`.`INVOICE_CUSTOMER`;
USE `shopdb`;
CREATE  OR REPLACE VIEW `INVOICE_CUSTOMER` AS
SELECT 
	INV_NUMBER, INV_DATE, INV_STATUS, INV_TOTAL, 
    CUS_CODE, CUS_LNAME, CUS_PHONE, CUS_WALLET,
    EMP_CODE
FROM INVOICE JOIN CUSTOMER ON INVOICE.CUS_CODE = CUSTOMER.CUS_CODE;

-- -----------------------------------------------------
-- View `shopdb`.`INVOICE_LINE`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `shopdb`.`INVOICE_LINE`;
USE `shopdb`;
CREATE  OR REPLACE VIEW `INVOICE_LINE` AS
SELECT 
	INV_NUMBER, INV_DATE, INV_STATUS, INV_TOTAL,
    LINE_NUMBER, P_CODE, P_DESCRIPT, LINE_UNITS, LINE_PRICE,
    EMP_CODE
FROM INVOICE 
...
DELIMITER $$
USE `shopdb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`INVENTORY_LOG_AFTER_INSERT` AFTER INSERT ON `INVENTORY_LOG` FOR EACH ROW
BEGIN
	 IF NEW.LOG_TYPE = 'RESTOCK' OR NEW.LOG_TYPE = 'GAIN' THEN
		UPDATE PRODUCT SET P_QOH = P_QOH + NEW.LOG_UNITS
...
USE `shopdb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`INVENTORY_LOG_BEFORE_UPDATE` BEFORE UPDATE ON `INVENTORY_LOG` FOR EACH ROW
BEGIN
	SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'CANNOT UPDATE INVENTORY LOG. INSERT A CORRECTING ROW.';
END$$

USE `shopdb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`INVENTORY_LOG_BEFORE_DELETE` BEFORE DELETE ON `INVENTORY_LOG` FOR EACH ROW
BEGIN
	SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'CANNOT DELETE INVENTORY LOG. INSERT A CORRECTING ROW.';
END$$

USE `shopdb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`CUSTOMER_BEFORE_DELETE` BEFORE DELETE ON `CUSTOMER` FOR EACH ROW
BEGIN
	SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'CANNOT DELETE CUSTOMER.';
END$$

USE `shopdb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`LINE_AFTER_INSERT` AFTER INSERT ON `LINE` FOR EACH ROW
BEGIN
	UPDATE INVOICE SET INV_TOTAL = SUM(LINE_UNITS * LINE_PRICE) WHERE INV_NUMBER = NEW.INV_NUMBER;
END$$
